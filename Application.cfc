...
<cfcomponent extends="fw1">
	...
	<cffunction name="setupApplication" output="false">
		...
		<cflock type="exclusive" timeout="50">
			<cfscript>
				...
				application.lib = CreateObject('component','#local.pc.getPackage()#.common.services.lib').init();
	
				// PayPal API Settings
				// For sandbox/developer testing:  https://developer.paypal.com | https://www.sandbox.paypal.com
				local.args = StructNew();
				local.args.APIAccount		= local.pc.getSetting('apiAccount');
				local.args.APIUsername 		= local.pc.getSetting('apiUsername');
				local.args.APIPassword 		= local.pc.getSetting('apiPassword');
				local.args.APISignature 	= local.pc.getSetting('apiSignature');
				local.args.useSandbox 		= local.pc.getSetting('useSandbox');
				local.args.useSDKCredentials = local.pc.getSetting('useSDKCredentials');
				local.args.useProxy 		= false;
				local.args.proxyName		= '';
				local.args.proxyPort		= '';
				local.args.notifyURL		= '';
	
				application.payPalService = CreateObject('component','#local.pc.getPackage()#.common.services.payPalService').init(argumentCollection=local.args);
	...
	<cffunction name="setupRequest">
		<cfscript>
			...
			// rc.isMura (if true, then accessing from front-end. if false, accessing from admin)
			request.context.isMura = isFrontEndRequest();
			request.context.isFrontEndRequest = isFrontEndRequest();

			request.context.lib = application.lib;
			request.context.$.lib = application.lib;
			request.context.payPalService = application.payPalService;
			request.context.$.payPalService = application.payPalService;

			// rc.URL
			request.context.url = StructNew();
			request.context.url.domain = getPageContext().getRequest().getServerName();
			request.context.url.scheme = getPageContext().getRequest().getScheme();
			request.context.url.base = request.context.url.scheme & '://' & request.context.url.domain;
			request.context.url.port = getPageContext().getRequest().getServerPort();
			if ( request.context.url.port neq '80' and request.context.url.port neq '443' ) {
				request.context.url.base = request.context.url.base & ':' & request.context.url.port;
			};			
			request.context.url.uri = getPageContext().getRequest().getRequestURI();
			if ( len(trim(getPageContext().getRequest().getQueryString())) ) {
				request.context.url.qs = '?' & getPageContext().getRequest().getQueryString();
			} else {
				request.context.url.qs = '';
			};
			request.context.url.absolute = request.context.url.uri & request.context.url.qs;
			request.context.url.full = request.context.url.base & request.context.url.absolute;
		</cfscript>
	</cffunction>
